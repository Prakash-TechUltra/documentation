...
establishment. In Odoo, there are several steps needed in configurations before you can successfully
use this feature.

The use of the guía de remisión electrónica electronic document is mandatory and required by |SUNAT|
for taxpayers who need to transfer their products, with the exception of those under the *Single
Simplified Regime* (régimen único simplificado or RUS).
...
The *Carrier* delivery guide type justifies the transportation service performed by the driver (or
carrier).
...
The *Private* transportation type option is used when the owner carries out the transfer of goods
using their own vehicles. In this case, a sender's delivery guide must be issued.
...
The following changes have been made in version 2.0 of the |GRE| delivery guide.
...
Version 2.0 requires additional information on the general configuration, vehicles, contacts, and
products. In the general configurations, it is now necessary to add new credentials that are
acquired in the |SUNAT| portal.

Flow
****

.. image:: peru/peru-flow-remision.png
   :align: center
   :alt: Flow for Guia de Remision emission.

#. Generate sales order
#. Validate delivery order
#. Generate waybill
#. Perform validation or rejection
#. Send CDR
#. Receive validated CDR on outbound order
#. Carrier travels with printed |GRE|
#. Deliver goods

Cancellations
*************

Currently, the |SUNAT| no longer uses the term "Anula", but now uses the term "Dar de baja" for
cancellations. Therefore, **both** the sender and the carrier can cancel the electronic waybill, so
...
and the :ref:`master data <peru/master_data>`, navigate to :menuselection:`Apps` and clear any
default filters set. Search for the `l10n_pe_edi_stock_20` module and click the :guilabel:`Activate`
button.

Next, you will need to obtain the *client ID* and *client secret* from |SUNAT|. To do so, follow the
...
The *operator* is the driver of the vehicle in cases where the delivery guide is through *private*
transport.
...
Once the transfer form is correctly validated by |SUNAT|, you will see the XML file generated in the
chatter. You can now print the delivery slip that will show the transfer details and the QR code
validated by |SUNAT|.
...
- `2325 - GrossWeightMeasure - El dato no cumple con el formato establecido "Hace falta el campo"
  ...
  This error occurs when the weight on the product is set as `0.00`. To fix this, you will need to
  cancel the waybill and recreate it. Make sure that you fix the weight on the product before
  creating the new waybill, or it will result in the same error.
...
- `Invalid content was found starting with element 'cac:BuyerCustomerParty'`
  ...
  According to the official documentation of the |SUNAT|'s waybill guide, the transfer reasons *03
  (sale with shipment to third party)* or *12 (others)* will not work correctly in Odoo, since you
  should not have an empty or blank customer.
